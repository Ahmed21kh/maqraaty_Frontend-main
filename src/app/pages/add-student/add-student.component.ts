...
export class AddStudentComponent {
 ...
 getNameOfOneStudent(id:string){
  ...
  handleAdd(title:string) {
    const formControl = new FormControl('',[Validators.min(10),Validators.pattern(/^\d{10}$/)])
  ...
  async getDataOfStudents() {
    ...
    this.appService.getStudent_code().subscribe({
      next: async(data) => {
        // console.log(data);
        console.log(data?.code);

        this.studentCode = await data?.code
  ...
  changelistofupload(event:NzUploadFile[]){
   console.log(event);
   console.log(this.myForm.value);
  }

 readonly handleRemoveFile:((file: NzUploadFile) => boolean | Observable<boolean>) = (event?:NzUploadFile) => {
    console.log(event);
